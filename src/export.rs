...
use crate::{constants::{self, TINTS, TINT_THRESHOLD}, ecology::{CellIndex, Ecosystem}, render::EcosystemRenderable};
...
pub(crate) fn export_hypsometric_color_map(height_map: [u8; constants::NUM_CELLS * 3], time_step: u32, path: &str) {
...
pub(crate) fn build_hypsometrically_tinted_map(height_map: [u8; constants::NUM_CELLS * 3]) -> [u8; constants::NUM_CELLS * 3] {
    let mut buffer = [0; constants::NUM_CELLS * 3];
    for i in (0..height_map.len()).step_by(3) {
        let height = height_map[i] as f32;
        let color = EcosystemRenderable::get_hypsometric_color_helper(height);
