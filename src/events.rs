...
impl Events {
    ...
    fn kill_trees(cell: &mut Cell) {
        if let Some(trees) = cell.get_trees_layer_mut() {
            let biovolume = trees.get_tree_biovolume();
...
#[cfg(test)]
mod tests {
    use crate::{ecology::{Cell, CellLayer, CellLayerKind, Trees, Vegetation, VegetationKind}, events::Events};
    #[test]
    fn kill_trees() {
        let layer = CellLayer {
        ...
        let biovolume = cell.get_tree_biovolume();
        ...
        assert!(dead_vegetation.volume == biovolume);
        ...
        let biovolume_2 = cell.get_tree_biovolume();
        ...
        assert!(dead_vegetation.is_some());
        let dead_vegetation = dead_vegetation.unwrap();
        assert!(dead_vegetation.volume == biovolume + biovolume_2);
    }
}